                    ...
                    <div class="col-xl-12">
                        ...
                        <div class="card">
                            <div class="card-header align-items-center d-flex">
                                <h4 class="card-title mb-0 flex-grow-1">Update Posts</h4>
                            ...
                            <div class="card-body">
                                <form action="{{ route('post.update', $post->id) }}" method="POST" class="row g-3 needs-validation" enctype="multipart/form-data">
                                    @csrf
                                    <div class="col-md-6">
                                        <label for="title" class="form-label">Title</label>
                                        <input type="text" name="title" class="form-control" id="title" value="{{ $post->title }}" required autocomplete="off">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select mb-3" aria-label="Default select example" name="category_id" id="category_id" required>
                                            ...
                                            @foreach ($pcategories as $pCat)
                                                <option value="{{ $pCat->id }}" @if( $pCat->id == $post->category_id ) selected @endif >{{ $pCat->name }}</option>
                                                @foreach (App\Models\Category::orderBy('name', 'asc')->where('is_parent', $pCat->id)->get() as $cCat )
                                                    <option value="{{ $cCat->id }}" @if( $cCat->id == $post->category_id ) selected @endif >-- {{ $cCat->name }}</option>
                                                @endforeach
                                            @endforeach
                                        </select>
                                        <div class="invalid-feedback">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="tags" class="form-control" id="tags" value="{{ $post->tags }}" required autocomplete="off">
                                        <div class="invalid-feedback">
                                            Please provide Meta Tags
                                        </div>
                                    </div>
                                    <div class="col-12">
                                        <button class="btn btn-primary" type="submit">Save changes Post</button>
                                    </div>
                                </form>
                            </div>
                        </div>
